...
def test_login(client, test_user, monkeypatch):
    ...
    monkeypatch.setattr(utils, "verify", verify_password_mock)
...
def test_signup(client, monkeypatch):
    ...
    monkeypatch.setattr(utils, "hash", get_password_hash_mock)
...
def test_wrong_password(client, test_db, test_user, test_password, monkeypatch):
    ...
    monkeypatch.setattr(utils, "verify", verify_password_failed_mock)

    response = client.post(
        "/api/login", data={"username": test_user.email, "password": "wrong"}
    )
...
def test_wrong_login(client, test_db, test_user, test_password):
    response = client.post(
        "/api/login", data={"username": "fakeuser", "password": test_password}
    )
    assert response.status_code == 403
